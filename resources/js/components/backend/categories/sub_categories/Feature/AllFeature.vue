                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        ...
                                                        <div class="card-tools">
                                                            <router-link to="addfeature" class="btn btn-block btn-outline-secondary">Add Feature </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Feature,i) in RetrieveFeature">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Feature.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Feature.name }}</td>
                                                                <td style="text-align: center">{{ Feature.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Feature.status)">
                                                                            {{ StatusAction(Feature.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Feature.status)"></i></a>

                                                                    <!--<a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatefeature/${Feature.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Feature.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    mounted(){
        this.$store.dispatch("GetFeature");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveFeature.length);
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteFeature/" + id).then((response) => {
                    this.$store.dispatch("GetFeature");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveFeature.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveFeature.forEach((Feature)=>{
                    if(this.selected.indexOf(Feature.id) === -1 ){
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedFeature/removeitems" ,{ FeatureData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Feature items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Feature/changestatus" ,{ FeatureData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Feature have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

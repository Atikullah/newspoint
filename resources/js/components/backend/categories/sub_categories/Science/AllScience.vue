                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        <h3 class="card-title "> Science List</h3>
                                                        <div class="card-tools">
                                                            <router-link to="addscience" class="btn btn-block btn-outline-secondary">Add Science </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Science,i) in RetrieveScience">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Science.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Science.name }}</td>
                                                                <td style="text-align: center">{{ Science.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Science.status)">
                                                                            {{ StatusAction(Science.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Science.status)"></i></a>

                                                                    <!--<a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatescience/${Science.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Science.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    mounted(){
        this.$store.dispatch("GetScience");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveScience.length);

        }
    },
    computed:{
        RetrieveScience(){
            return this.$store.getters.Science;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteScience/" + id).then((response) => {
                    this.$store.dispatch("GetScience");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveScience.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveScience.forEach((Science)=>{
                    if(this.selected.indexOf(Science.id) === -1 ){
                        this.selected.push(Science.id);
                    }
                });
            }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedScience/removeitems" ,{ ScienceData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Science items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Science/changestatus" ,{ ScienceData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Science have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        <h3 class="card-title "> Lifestyle List</h3>
                                                        <div class="card-tools">
                                                            <router-link to="addlifestyle" class="btn btn-block btn-outline-secondary">Add Lifestyle </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Lifestyle,i) in RetrieveLifestyle">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Lifestyle.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Lifestyle.name }}</td>
                                                                <td style="text-align: center">{{ Lifestyle.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Lifestyle.status)">
                                                                            {{ StatusAction(Lifestyle.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Lifestyle.status)"></i></a>

                                                                    <!--<a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatelifestyle/${Lifestyle.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Lifestyle.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    mounted(){
        this.$store.dispatch("GetLifestyle");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveLifestyle.length);

        }
    },
    computed:{
        RetrieveLifestyle(){
            return this.$store.getters.Lifestyle;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteLifestyle/" + id).then((response) => {
                    this.$store.dispatch("GetLifestyle");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveLifestyle.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveLifestyle.forEach((Lifestyle)=>{
                    if(this.selected.indexOf(Lifestyle.id) === -1 ){
                        this.selected.push(Lifestyle.id);
                    }
                });
            }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedLifestyle/removeitems" ,{ LifestyleData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Lifestyle items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Lifestyle/changestatus" ,{ LifestyleData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Lifestyle have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

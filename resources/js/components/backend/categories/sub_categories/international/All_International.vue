                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        ...
                                                        <div class="card-tools">
                                                            <router-link to="addCountry" class="btn btn-block btn-outline-secondary">Add Country</router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(International,i) in RetrieveInternational">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="International.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ International.name }}</td>
                                                                <td style="text-align: center">{{ International.hit }}</td>
                                                                <td style="text-align: center">{{ International.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(International.status)">
                                                                            {{ StatusAction(International.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(International.status)"></i></a>

<!--                                                                    <a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updateinternational/${International.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(International.id)"><i class="fas fa-trash"></i></button>
...
name: "All_International",
    ...
    mounted(){
        this.$store.dispatch("GetInternational");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveInternational.length);

        }
    },
    computed:{
        RetrieveInternational(){
            return this.$store.getters.International;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteInternational/" + id).then((response) => {
                    this.$store.dispatch("GetInternational");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveInternational.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveInternational.forEach((International)=>{
                    if(this.selected.indexOf(International.id) === -1 ){
                        this.selected.push(International.id);
                    }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedInternational/removeitems" ,{ InternationalData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  International items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/International/changestatus" ,{ InternationalData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  International have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

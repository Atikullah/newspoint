                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        <h3 class="card-title ">All bangladesh List</h3>
                                                        <div class="card-tools">
                                                            <router-link to="addbangladesh" class="btn btn-block btn-outline-secondary">Add Resource</router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Bangladesh,i) in RetrieveBangladesh">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Bangladesh.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Bangladesh.name }}</td>
                                                                <td style="text-align: center">{{ Bangladesh.name }}</td>
                                                                <td style="text-align: center">{{ Bangladesh.hit }}</td>
                                                                <td style="text-align: center">{{ Bangladesh.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Bangladesh.status)">
                                                                            {{ StatusAction(Bangladesh.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Bangladesh.status)"></i></a>

                                                                    <!--                                                                    <a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatebangladesh/${Bangladesh.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Bangladesh.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    mounted(){
        this.$store.dispatch("GetBangladesh");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveBangladesh.length);

        }
    },
    computed:{
        RetrieveBangladesh(){
            return this.$store.getters.Bangladesh;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteBangladesh/" + id).then((response) => {
                    this.$store.dispatch("GetBangladesh");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveBangladesh.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveBangladesh.forEach((Bangladesh)=>{
                    if(this.selected.indexOf(Bangladesh.id) === -1 ){
                        this.selected.push(Bangladesh.id);
                    }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedBangladesh/removeitems" ,{ BangladeshData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Bangladesh items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Bangladesh/changestatus" ,{ BangladeshData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Data have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

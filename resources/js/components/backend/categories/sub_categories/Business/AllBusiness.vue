                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        ...
                                                        <div class="card-tools">
                                                            <router-link to="addbusiness" class="btn btn-block btn-outline-secondary">Add Business </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Business,i) in RetrieveBusiness">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Business.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Business.name }}</td>
                                                                <td style="text-align: center">{{ Business.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Business.status)">
                                                                            {{ StatusAction(Business.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Business.status)"></i></a>

                                                                    <!--                                                                    <a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatebusiness/${Business.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Business.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    watch:{
        selected: function (Selected) {
            ...
            this.Allselected = (Selected.length === this.RetrieveBusiness.length);
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteBusiness/" + id).then((response) => {
                    this.$store.dispatch("GetBusiness");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveBusiness.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveBusiness.forEach((Business)=>{
                    if(this.selected.indexOf(Business.id) === -1 ){
                        this.selected.push(Business.id);
                    }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedBusiness/removeitems" ,{ BusinessData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Business items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Business/changestatus" ,{ BusinessData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Business have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

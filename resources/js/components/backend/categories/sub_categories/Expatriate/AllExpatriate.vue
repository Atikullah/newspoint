        ...
        <section class="content">
            ...
            <div class="card" style="margin-top: -20px;">
                <div class="card-header">
                    <h3 class="card-title">Expatriate</h3>
                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        <h3 class="card-title "> Expatriate List</h3>
                                                        <div class="card-tools">
                                                            <router-link to="addexpatriate" class="btn btn-block btn-outline-secondary">Add Expatriate </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Expatriate,i) in RetrieveExpatriate">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Expatriate.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Expatriate.name }}</td>
                                                                <td style="text-align: center">{{ Expatriate.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Expatriate.status)">
                                                                            {{ StatusAction(Expatriate.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Expatriate.status)"></i></a>

                                                                    <!--<a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updateexpatriate/${Expatriate.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Expatriate.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    mounted(){
        this.$store.dispatch("GetExpatriate");
    },
    watch:{
        selected: function (Selected) {
            this. IsSelected = (Selected.length > 0);
            this.Allselected = (Selected.length === this.RetrieveExpatriate.length);

        }
    },
    computed:{
        RetrieveExpatriate(){
            return this.$store.getters.Expatriate;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteExpatriate/" + id).then((response) => {
                    this.$store.dispatch("GetExpatriate");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveExpatriate.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveExpatriate.forEach((Expatriate)=>{
                    if(this.selected.indexOf(Expatriate.id) === -1 ){
                        this.selected.push(Expatriate.id);
                    }
                });
            }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedExpatriate/removeitems" ,{ ExpatriateData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Expatriate items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Expatriate/changestatus" ,{ ExpatriateData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Expatriate have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

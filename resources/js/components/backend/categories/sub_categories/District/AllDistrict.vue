                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        ...
                                                        <div class="card-tools">
                                                            <router-link to="adddistrict" class="btn btn-block btn-outline-secondary">Add District</router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(District,i) in RetrieveDistrict">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="District.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ District.name }}</td>
                                                                <td style="text-align: center">{{ District.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(District.status)">
                                                                            {{ StatusAction(District.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(District.status)"></i></a>

                                                                    <!-- <a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatedistrict/${District.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(District.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    watch:{
        selected: function (Selected) {
            ...
            this.Allselected = (Selected.length === this.RetrieveDistrict.length);
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteDistrict/" + id).then((response) => {
                    this.$store.dispatch("GetDistrict");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveDistrict.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveDistrict.forEach((District)=>{
                    if(this.selected.indexOf(District.id) === -1 ){
                        this.selected.push(District.id);
                    }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedDistrict/removeitems" ,{ DistrictData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  District items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/District/changestatus" ,{ DistrictData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  District have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });

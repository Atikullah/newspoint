                                ...
                                <section class="content" >
                                    <div class="container-fluid">
                                        ...
                                        margin-left: 20px;margin-right: 20px;margin-top:20px;} ">
                                                        ...
                                                        <div class="card-tools">
                                                            <router-link to="addreligion" class="btn btn-block btn-outline-secondary">Add Religion </router-link>
                                                    ...
                                                    <div class="card-body"  style="margin-top: -20px; ">
                                                        <table id="example1" class="table table-bordered table-striped">
                                                            ...
                                                            <tr v-for="(Religion,i) in RetrieveReligion">
                                                                <td style="text-align: center">
                                                                    <input type="checkbox" :value="Religion.id" v-model="selected" style="margin-left:-5px;margin-top:12px;">
                                                                </td>


                                                                <td style="text-align: center">{{++i}}</td>
                                                                <td style="text-align: center">{{ Religion.name }}</td>
                                                                <td style="text-align: center">{{ Religion.created_at | time }}</td>
                                                                <td><span class="badge" style="font-size:17px;font-family:none;padding: 10px;margin-left:20px;
                                                                            width:100px;text-align:center;border-radius:18px;" :class="StatusColore(Religion.status)">
                                                                            {{ StatusAction(Religion.status) }}</span></td>

                                                                <td style="text-align: center">

                                                                    <a href="#"<i class="fas" :class="StatusIcon(Religion.status)"></i></a>

                                                                    <!--<a href="#" class="btn btn-info"><i class="fas fa-eye"></i></a>-->

                                                                    <router-link :to="`/updatereligion/${Religion.name}`" class="btn btn-primary"><i class="fas fa-edit"></i></router-link>

                                                                    <button type="button" class="btn btn-danger" @click="remove(Religion.id)"><i class="fas fa-trash"></i></button>
...
export default {
    ...
    watch:{
        selected: function (Selected) {
            ...
            this.Allselected = (Selected.length === this.RetrieveReligion.length);

        }
    },
    computed:{
        RetrieveReligion(){
            return this.$store.getters.Religion;
        }
    },
    ...
    methods: {
        ...
        remove: function (id){
            this.Confirm(() =>{
                axios.get("DeleteReligion/" + id).then((response) => {
                    this.$store.dispatch("GetReligion");
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
        ...
        emptyData(){
            return (this.RetrieveReligion.length < 1);
        },
        ...
        Select: function (event) {
            if(event.target.checked === false){
                ...
                this.RetrieveReligion.forEach((Religion)=>{
                    if(this.selected.indexOf(Religion.id) === -1 ){
                        this.selected.push(Religion.id);
                    }
                });
            }
        ...
        RemoveItem: function (selected) {
            let this_ = this;
            this.Confirm(() =>{
                axios.post("/selectedReligion/removeitems" ,{ ReligionData: selected }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalDelete + '  Religion items  has been deleted successfully'),
                        showConfirmButton: false,
                        timer: 1000
                    });
        ...
        changeStatus: function (selected,status) {
            ...
            this.Confirm(() =>{
                axios.post("/Religion/changestatus" ,{ ReligionData: selected, status: status }).then((response) => {
                    ...
                    Swal.fire({
                        position: 'top-end',
                        icon: 'success',
                        title: (response.data.TotalCount + '  Religion have been successfully '+msg),
                        showConfirmButton: false,
                        timer: 1500
                    });
